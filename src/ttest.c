    	...
    	while (b <= X && j < cols)
    	{
			if (array[i][j] == 1)
			{
				if (i == 0 && j == 0)                          // Left upper corner
             		mlx_put_image_to_window(mlx_ptr, mlx_win, img_corner_left_up, b, a);
            	else if (i == 0 && j == cols - 1)              // Right upper corner
                	mlx_put_image_to_window(mlx_ptr, mlx_win, img_corner_right_up, b, a);
            	else if (i == rows - 1 && j == 0)              // Left lower corner
                	mlx_put_image_to_window(mlx_ptr, mlx_win, img_corner_left_down, b, a);
            	else if (i == rows - 1 && j == cols - 1)       // Right lower corner
                	mlx_put_image_to_window(mlx_ptr, mlx_win, img_corner_right_down, b, a);
            	else if (i == 0)                               // Upper side
                	mlx_put_image_to_window(mlx_ptr, mlx_win, img_side_up, b, a);
            	else if (i == rows - 1)                        // Lower side
                	mlx_put_image_to_window(mlx_ptr, mlx_win, img_side_down, b, a);
            	else if (j == 0)                               // Left side
                	mlx_put_image_to_window(mlx_ptr, mlx_win, img_side_left, b, a);
            	else if (j == cols - 1)                        // Right side
                	mlx_put_image_to_window(mlx_ptr, mlx_win, img_side_right, b, a);
        	}
			else if (array[i][j] == 0)
			{
				if (array[i - 1][j] == 1 && array[i][j - 1] == 1)      // Left upper corner
             		mlx_put_image_to_window(mlx_ptr, mlx_win, img_road_corner_left_up, b, a);
				else if (array[i - 1][j] == 1 && array[i][j + 1] == 1) // Right upper corner
                	mlx_put_image_to_window(mlx_ptr, mlx_win, img_road_corner_right_up, b, a);
            	else if (array[i + 1][j] == 1 && array[i][j - 1] == 1) // Left lower corner
                	mlx_put_image_to_window(mlx_ptr, mlx_win, img_road_corner_left_down, b, a);
				else if (array[i + 1][j] == 1 && array[i][j + 1] == 1) // Right lower corner
                	mlx_put_image_to_window(mlx_ptr, mlx_win, img_road_corner_right_down, b, a);
				else if (array[i - 1][j] == 1 && array[i + 1][j] == 0) // Upper side
                	mlx_put_image_to_window(mlx_ptr, mlx_win, img_road_side_up, b, a);
				else if (array[i + 1][j] == 1 && array[i - 1][j] == 0) // Lower side
                	mlx_put_image_to_window(mlx_ptr, mlx_win, img_road_side_down, b, a);
				else if (array[i][j - 1] == 1 && array[i][j + 1] == 0) // Left side
                	mlx_put_image_to_window(mlx_ptr, mlx_win, img_road_side_left, b, a);
				else if (array[i][j - 1] == 0 && array[i][j + 1] == 1) // Right side
                	mlx_put_image_to_window(mlx_ptr, mlx_win, img_road_side_right, b, a);
				else
					mlx_put_image_to_window(mlx_ptr, mlx_win, img_inside_road, b, a);
			}
